...
export class CourseSessionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.courseSessionService
      ...
      .subscribe(
        (res: HttpResponse<ICourseSession[]>) => {
  ...
  protected onSuccess(data: ICourseSession[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/course-session'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
