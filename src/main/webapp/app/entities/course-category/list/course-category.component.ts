...
export class CourseCategoryComponent implements OnInit {
  ...
  trackId(index: number, item: ICourseCategory): number {
    return item.id!;
  }

  delete(courseCategory: ICourseCategory): void {
    const modalRef = this.modalService.open(CourseCategoryDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ICourseCategory[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
