...
export class CourseLevelComponent implements OnInit {
  ...
  trackId(index: number, item: ICourseLevel): number {
    return item.id!;
  }

  delete(courseLevel: ICourseLevel): void {
    const modalRef = this.modalService.open(CourseLevelDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.courseLevel = courseLevel;
  ...
  protected onSuccess(data: ICourseLevel[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
